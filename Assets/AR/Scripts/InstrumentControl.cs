...
public class InstrumentControl : MonoBehaviour
{
    ...
    public void UserCueAction()
    {
        if(m_MyAudioSource.isPlaying)
        {
            if(Math.Abs(cues[cueIndex] - m_MyAudioSource.time) < 0.5){
            ...
            } /* else if(Math.Abs(cues[cueIndex] - m_MyAudioSource.time) > 0.5){
                if(cueIndex % 2 == 0){
                    if(m_MyAudioSource.time > cues[cueIndex]){
                ...
                if(m_MyAudioSource.mute){
                    if(cueIndex % 2 == 0){
                        ...
                        Debug.Log("Nothing to cue in:" + clip);
                    } else{
                        ...
                        Debug.Log("Cued back in when should be playing:" + clip);
                    }
                } else{
                    if(cueIndex % 2 != 0){
                        m_MyAudioSource.mute = true;
                        ...
                        Debug.Log("You've cut off the musicians:" + clip);
                    }
                }
            }
        }
